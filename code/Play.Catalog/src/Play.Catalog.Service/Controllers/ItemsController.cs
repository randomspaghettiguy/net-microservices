...
namespace Play.Catalog.Service.Controllers
{
    ...
    public class ItemsController : ControllerBase
    {
        ...
        public ActionResult<ItemDto> Post(CreateItemDto createItemDto)
        {
            ...
            return CreatedAtAction(nameof(GetById), new { id = item.Name }, item);
        }

        [HttpPut("{id}")]
        public IActionResult Post(Guid id, UpdateItemDto updateItemDto)
        {
            var existingItem = items.Where(item => item.Id == id)
                                    .SingleOrDefault();
